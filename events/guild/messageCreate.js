...
module.exports = async (client, message) => {
    ...
    if(cmd.length == 0){
        if(mPrefix.includes(client.user.id)){
            message.reply({embeds: [new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, client.user.displayAvatarURL()).setTitle(`:thumbsup: **My Prefix here, is __\`${prefix}\`__**`)]})
        }
        return;
    }
