...
module.exports = {
  name: "passage", //the command name for the Slash Command
  description: "Donne les prochains passage Tisséo", //the command description for Slash Command Overview
  cooldown: 1,
  ...
  run: async (client, interaction) => {
    try {
      ...
      let ligne = options.getString("ligne");
      let arret = options.getString("arrêt");

      const stopPointId = search_id_stop(arret, arret_json)
      const lineId = search_id_line(ligne, lignes_json)

      if (!stopPointId || !lineId) {
        return interaction.reply('Ligne / Arrêt inconnue')
      }
      ...
      request(lien, options_json, (error, res, body) => {
        ...
        if (!error && res.statusCode == 200) {
          ...
          embed.setTitle(`Prochains passages pour la ligne ${ligne} à l'arrêt ${arret}`)
