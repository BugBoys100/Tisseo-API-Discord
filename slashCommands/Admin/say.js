...
module.exports = {
  ...
  description: "Envoie du texte dans le chat", //the command description for Slash Command Overview
  cooldown: 5,
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
	//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		...
		{"String": { name: "text", description: "Que dois-je envoyer ? [ +n+ = Newline ]", required: true }}, //to use in the code: interacton.getString("title")
		//{"User": { name: "ping_a_user", description: "To Ping a user lol", required: false }}, //to use in the code: interacton.getUser("ping_a_user")
		{"Channel": { name: "in_where", description: "Dans quel salon je dois l'envoyer ?", required: false }}, //to use in the code: interacton.getChannel("what_channel")
  ...
  run: async (client, interaction) => {
    try{
		...
		await interaction.reply({content: `Envoi du message...`, ephemeral: true}); 
		//SEND THE EMBED!
		await channel.send({content: String(Text).substr(0, 2000).split("+n+").join("\n")});
		//Edit the reply
		interaction.editReply({content: `✅ Message envoyé !`, ephemeral: true}); 
    } catch (e) {
