...
module.exports = {
  ...
  description: "Envoie un embed dans le chat", //the command description for Slash Command Overview
  cooldown: 5,
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
	//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		...
		{"String": { name: "description", description: "Que sera la description de l'embed ? [ +n+ = NewLine ]", required: true }}, //to use in the code: interacton.getString("description")
  ...
  run: async (client, interaction) => {
    try{
		...
		await interaction.reply({content: `Envoi de l'embed...`, ephemeral: true}); 
		//SEND THE EMBED!
		await channel.send({embeds: [embed]});
		//Edit the reply
		interaction.editReply({content: `✅ Embed envoyé !`, ephemeral: true}); 
    } catch (e) {
