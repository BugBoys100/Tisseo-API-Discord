...
module.exports = {
  ...
  aliases: ["latence"], //the command aliases for helpcmd [OPTIONAL]
  cooldown: 5, //the command cooldown for execution & for helpcmd [OPTIONAL]
  usage: "ping", //the command usage for helpcmd [OPTIONAL]
  description: "Donne des infos sur la vitesse du bot", //the command description for helpcmd [OPTIONAL]
    ...
    try{
      ...
      message.reply({ embeds: [new MessageEmbed()
        ...
        msg.edit({embeds: [new MessageEmbed()
          .setColor(ee.color)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle(`🏓 Ping : \`${Math.round(Date.now() - date)}ms\`\n\n:robot: Latence de l'api : \`${Math.round(client.ws.ping)}ms\``)
        ]});
      })
    } catch (e) {
