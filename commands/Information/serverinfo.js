...
module.exports = {
  ...
  description: "Montre les informations du sereur", //the command description for helpcmd [OPTIONAL]
  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    try {
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .addField("❱ Tu as rejoins le", "\`" + moment(message.member.joinedTimestamp).format("DD/MM/YYYY") + "\`\n" + "`"+ moment(message.member.joinedTimestamp).format("hh:mm:ss") +"`", true)
      
        .addField("❱ Tout les salons", "👁‍🗨 \`" + message.guild.channels.cache.size + "\`", true)
        .addField("❱ Salons Textuels", "💬 \`" + message.guild.channels.cache.filter(channel => channel.type == "text").size + "\`", true)
        .addField("❱ Salons Vocaux", "🔈 \`" + message.guild.channels.cache.filter(channel => channel.type == "voice").size + "\`", true)
       
        .addField("❱ Tout les Membres", "😀 \`" + message.guild.memberCount + "\`", true)
        .addField("❱ Tout les Humains", "👤 \`" + message.guild.members.cache.filter(member => !member.user.bot).size + "\`", true)
        .addField("❱ Tout les Bots", "🤖 \`" + message.guild.members.cache.filter(member => member.user.bot).size + "\`", true)
        ...
        .addField("❱ Total des Boosts", "\`" + message.guild.premiumSubscriptionCount + "\`", true)
        .addField("❱ Level de Boost", "\`" + boostlevel + "\`", true)
