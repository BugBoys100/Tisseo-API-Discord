...
module.exports = {
  ...
  description: "Montre des infos sur un rôle", //the command description for helpcmd [OPTIONAL]
    ...
    try {   
      ...
      if(args[0]){
        try{
          ...
          if(!e) return message.reply("IMPOSSIBLE DE TROUVER LE RÔLE")
          return message.reply(e)
        }
      }else{
        return message.reply("❌ Merci de recommencer mais d'ajouter un Role/Rolename/Roleid");
      }
      if(!role || role == null || role.id == null || !role.id) message.reply("❌ Impossible de trouver le rôle")
        //create the EMBED
        const embeduserinfo = new MessageEmbed()
        embeduserinfo.setThumbnail(message.guild.iconURL({ dynamic: true, size: 512 }))
        embeduserinfo.setAuthor("Informations sur :   " + role.name, message.guild.iconURL({ dynamic: true }), "https://discord.gg/FQGXbypRf8")
        ...
        embeduserinfo.addField('**❱ Epinglé : **',`\`${role.hoist ? "✔️" : "❌"}\``,true)
